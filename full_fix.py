...
c = c.replace(
    ...
    '# QVideoWidget 已移除（VLC 迁移）'
)
print('[1] imports OK')

# ============================================================================
# 2. video_widget 改为 QWidget
# ============================================================================
c = c.replace(
...
print('[5] signal connections removed')

# ============================================================================
# 6. media_player → _vlc_player 替换（所有位置）
# ============================================================================
# 注意：顺序很重要，先替换长字符串再替换短字符串
replacements = [
    ...
    ('self.media_player.setSource(', '# setSource removed (VLC)'),
...
for old, new in replacements:
    c = c.replace(old, new)
print('[6] media_player → _vlc_player OK')

# ============================================================================
# 7. 替换状态变化处理方法
# ============================================================================
# _on_state_changed → _on_vlc_state_changed
...
print('[7] _on_state_changed OK')

# ============================================================================
# 8. 添加 _poll_vlc_state 和 _get_display_title
# ============================================================================
poll_method = '''
...
for i, l in enumerate(lines):
    if '    def _on_vlc_state_changed(self' in l:
        ...
        break
c = '\n'.join(lines)
print('[8] _poll_vlc_state + _get_display_title OK')
...
# ============================================================================
# 11. 添加音量控制和字幕开关按钮（在 time_label 后）
# ============================================================================
vol_code = '''
        # 音量控制
...
print('[11] volume + subtitle_toggle_btn OK')

# ============================================================================
# 12. 添加辅助方法（进度条点击 + 音量 + 字幕开关）
# ============================================================================
helpers = '''
    # ── 进度条点击定位 ─────────────────────────────────────────────────
...
lines = c.split('\n')
for i, l in enumerate(lines):
    if '    def _get_display_title(self' in l:
        lines.insert(i, helpers)
        break
c = '\n'.join(lines)
print('[12] helper methods OK')
...
print('[13] slider click OK')

# ============================================================================
# 14. subtitle_panel 改为 self
# ============================================================================
...
print('[14] subtitle_panel self OK')

# ============================================================================
# 15. history_panel 最小宽度
# ============================================================================
c = c.replace(
    '        history_panel = QWidget()\n        history_panel.setStyleSheet',
    '        history_panel = QWidget()\n        history_panel.setMinimumWidth(180)\n        history_panel.setStyleSheet'
)
print('[15] history min width OK')

# ============================================================================
# 16. _add_to_history 不置顶
# ============================================================================
c = c.replace(
    'self._video_history = [entry] + remaining',
    '# 不置顶：原地更新或追加末尾'
)
print('[16] add_history no-reorder OK')

# ============================================================================
# 17. _stop_playback 更新
# ============================================================================
c = c.replace(
    '''    @Slot()
...
print('[18] enable controls OK')

# ============================================================================
# 19. closeEvent VLC 清理
# ============================================================================
c = c.replace(
    '        self.media_player.stop()',
    '        if hasattr(self, "_vlc_player") and self._vlc_player:\n            self._vlc_player.stop()\n        if hasattr(self, "_vlc_instance") and self._vlc_instance:\n            self._vlc_instance.release()'
)
print('[19] closeEvent OK')

# ============================================================================
# 20. 历史双击时的 VLC 检查
# ============================================================================
c = c.replace(
    'if self._vlc_player.get_state() == QMediaPlayer.PlaybackState.PlayingState:',
    'if self._vlc_player.get_state() == vlc.State.Playing:'
)
print('[20] history double-click VLC OK')

# ============================================================================
# 21. 移除 @Slot 装饰器
# ============================================================================
c = c.replace('    @Slot()\n    def _toggle_playback', '    def _toggle_playback')
c = c.replace('    @Slot()\n    def _start_download', '    def _start_download')
c = c.replace('    @Slot()\n    def _stop_playback', '    def _stop_playback')
c = c.replace('    @Slot(int)\n    def _seek_position', '    def _seek_position')
c = c.replace('    @Slot(int)\n    def _on_position_changed', '    def _on_position_changed')
c = c.replace('    @Slot(int)\n    def _on_duration_changed', '    def _on_duration_changed')
c = c.replace('    @Slot(float)\n    def _on_download_progress', '    def _on_download_progress')
c = c.replace('    @Slot(object)\n    def _on_download_finished', '    def _on_download_finished')
c = c.replace('    @Slot(list)\n    def _on_transcribe_done', '    def _on_transcribe_done')
c = c.replace('    @Slot(str)\n    def _on_transcribe_error', '    def _on_transcribe_error')
c = c.replace('    @Slot(str)\n    def _on_text_segment', '    def _on_text_segment')
c = c.replace('    @Slot(object)\n    def _on_history_item_double_clicked', '    def _on_history_item_double_clicked')
c = c.replace('    @Slot()\n    def _on_history_clear', '    def _on_history_clear')
c = c.replace('    @Slot()\n    def _export_srt', '    def _export_srt')
c = c.replace('    @Slot()\n    def _export_txt', '    def _export_txt')
print('[21] @Slot decorators removed')

# ============================================================================
# 22. QSplitter handle 样式
# ============================================================================
c = c.replace(
...
print('[22] QSplitter style OK')

# ============================================================================
# 23. _on_position_changed 添加节流
# ============================================================================
c = c.replace(
